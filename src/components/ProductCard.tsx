 ...
 * Single product card.
 *
 * Shared rendering for the Shop's product surfaces. Used by:
 *   - `/shop` main grid via ShopGrid
 *   - `/shop/[pillar]/[subcategory]` direct grid
 *
 * Features:
 *   - touch-swipe through `product.images` on mobile
 *   - hover-revealed prev / next buttons + position counter on desktop
 *   - inline dot indicators when more than one image is present
 *   - sold-out signal replaces the price when the live-pricing layer
 *     reports sold-out
 *
 * The card never recomputes a price — pass in `price` already resolved
 * from the pricing layer (`fetchAllPrices` / `fetchProductPrice`).
 */
...
export function ProductCard({
  ...
  product: ShopProduct;
  /** CSS aspect-ratio value for the image frame (e.g. "4 / 5"). */
  aspect?: string;
  /** Resolved price for this product. May be live, manual fallback,
   *  or a sold-out signal. The card never recomputes — it just renders. */
  ...
  // Tailwind classes shared by both arrow buttons.
  const arrowBase =
