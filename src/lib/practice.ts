...
/**
 * Practice — the fourth content pillar.
 *
 * Lifestyle, spiritual, wellness, exploration. First-person register.
 * Each piece refuses a binary and declares a standard.
 *
 * This module mirrors `lib/living.ts` deliberately: same frontmatter
 * shape, same minimal markdown renderer. The two pillars share an
 * editorial vocabulary; sharing implementation keeps that vocabulary
 * legible as the codebase grows. If Practice diverges later (richer
 * frontmatter, structured sections), fork at that point — not before.
 */
...
async function readArticle(filename: string): Promise<PracticeArticle> {
...
export async function getAllPracticeSlugs(): Promise<string[]> {
  try {
...
export async function getAllPracticeArticles(): Promise<PracticeArticle[]> {
  try {
    const entries = await fs.readdir(CONTENT_DIR);
...
export async function getPracticeArticleBySlug(
  slug: string
): Promise<PracticeArticle | undefined> {
  try {
