...
def torso(mask, shL,shR,hpL,hpR, waist=0.86):
    ...
    # densify with a light spline feel via polygon of these 6 + fill
    poly=np.array([[_x(p),_y(p)] for p in pts],np.int32)
    cv2.fillPoly(mask,[poly],255)
    # round the shoulders & hips
    for p,r in [(shL,0.055),(shR,0.055),(hpL,0.07),(hpR,0.07)]:
...
def render(J, R, smooth=True):
    ...
    # torso + neck + head
    torso(m,g('shL'),g('shR'),g('hpL'),g('hpR'),R.get('waist',0.86))
