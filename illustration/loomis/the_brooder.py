"""
THE BROODER · WEIGHT BEFORE FEATURE  (Sketch 101, 2026-05-30)
Professor: "The head tilts; the person doesn't. The geometry says '/', the psychology doesn't
yet. This is where visual gravity enters — not construction, not anatomy, not proportion.
WEIGHT." Assignment: one head, make it feel HEAVY before a single feature, using only
silhouette, tilt, mass distribution, proportion. Two of his structural notes folded in:
 - DIFFERENT ARCHITECTURE, not bigger knobs: the brooder's organizing principle is a
   FORWARD-PROJECTING BROW MASS over a RECEDING LOWER FACE (heavy front-top, weak chin).
 - the model is SYMMETRIC, so weight needs DESIGN to enter: a gravitational DEFORM breaks
   symmetry — the lower mass settles and leans to one side, the heavy side sags, and the
   head hangs forward, so the MASS leans rather than the geometry merely rotating.
Still line-only, no features. The small pair at the bottom is the lesson itself: the same
skull TILTED (geometry only) vs LEANING (mass settled).
"""
import os, math
import numpy as np
...
CHIN={'chT_c','chT_r','ch_c','ch_r','lipB_c','lipB_r'}

def make_faces(ch):
    g=lambda k:ch.get(k,1.0); g0=lambda k:ch.get(k,0.0)
    ...
    brow_fwd,chin_recede=g0('brow_fwd'),g0('chin_recede')
    def ty(y):
    ...
    def warpV(name,x,y,z):
        ...
        if name.startswith('browC'): nz=z+(brow-1)*16+brow_fwd
        ...
        if name in CHIN: nz=nz-chin_recede
        return (nx,ny,nz)
    def warpR(r):
...
def gravity(LEAN,SAG,ASYM):
    """settle the lower mass: it leans to +x and sags down, the heavy (+x) side more.
    this is the DESIGN that makes the mass lean rather than the geometry only rotating."""
    def d(p):
        x,y,z=p; t=max(0.0,min(1.0,(y+110)/240.0)); t2=t*t
        x=x+LEAN*t2*60
        y=y+SAG*t2*16 + ASYM*max(0.0,x)/60.0*t*14
...
def raster_head(faces_src,cx,cy,sc,yaw,pitch,roll=0,foc=FOC,deform=None,minp=28,silw=1):
    Rm=Rz(rad(roll))@Ry(rad(yaw))@Rx(rad(pitch)); f=foc
    def proj(p): s=f/(f-p[2]); return (cx+p[0]*sc*s, cy+p[1]*sc*s)
    faces=[]; allx=[]; ally=[]
    for pts,cull in faces_src:
        rp=[Rm@(deform(p) if deform else p) for p in pts]; nrm=newell(rp)
...
F_TTL=font(20); F_SM=font(8.5); F_IT=font(11); F_CAP=font(10)
