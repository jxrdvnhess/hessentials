"""
FIVE PEOPLE · OVERLAP  (the human before anatomy — 5, 2026-05-30)
Professor: "Connection creates an organism. OVERLAP creates solidity. Same five people, do not
change a single mass, no anatomy, no detail — only let the forms occupy the same space: ribcage
overlaps pelvis, shoulder mass overlaps ribcage, arm partially disappears behind the torso, one
form passes in front of another." So the only change from the connected page is HIDDEN-SURFACE:
each mass paper-fills its own silhouette (opaque) and the figure is drawn BACK-TO-FRONT, so a
nearer mass occludes the farther ones. Transparent wireframe -> interlocking solids.
Near side = +x (right) under the -30 camera; far side = -x.
Draw order: spine -> far arm -> legs -> pelvis -> ribcage -> neck -> head -> shoulder/clavicle
-> near arm. (Masses identical to five_people_connected.py.)
"""
import os, math
import numpy as np
...
W,H,SS=1480,1080,2
PAPER=(237,232,223); INK=(46,46,58); CHAIN=(40,40,52); FAINT=(196,193,186); GREY=(120,118,112); LABEL=(96,94,90)
...
def fillhull(samples3d,ox,oy):
    poly=[(x*SS,y*SS) for x,y in hull([projP(p,ox,oy) for p in samples3d])]
    if len(poly)>=3: D.polygon(poly,fill=PAPER)

def ellip_pts(center,radii,rot):
    rx,ry,rz=radii; c=V(*center); out=[]
    for i in range(7):
        th=math.pi*(i+0.5)/7
        for j in range(18):
            ph=2*math.pi*j/18
            out.append(c+rot@V(rx*math.sin(th)*math.cos(ph),ry*math.cos(th),rz*math.sin(th)*math.sin(ph)))
    return out
def sphere_pts(center,r): return ellip_pts(center,(r,r,r),EYE)
def box_pts(center,half,rot):
    hx,hy,hz=half; c=V(*center)
    return [c+rot@V(sx*hx,sy*hy,sz*hz) for sx in(-1,1) for sy in(-1,1) for sz in(-1,1)]
def seg_pts(a,b,ra,rb):
    ...
    for c,r in ((a,ra),(b,rb)):
        for k in range(12):
            th=2*math.pi*k/12; out.append(c+r*(math.cos(th)*u+math.sin(th)*v))
    return out

def ellipsoid(center,radii,rot,ox,oy,w=1.9,contour=True):
    fillhull(ellip_pts(center,radii,rot),ox,oy)
    CV(hull([projP(p,ox,oy) for p in ellip_pts(center,radii,rot)]),w,INK,closed=True)
    if contour:
        rx,ry,rz=radii; c=V(*center)
...
def boxsolid(center,half,rot,ox,oy,w=1.7):
    cw=box_pts(center,half,rot); fillhull(cw,ox,oy)
    ...
    for a in range(8):
        for b in range(a+1,8):
            if sum(1 for k in range(3) if combo[a][k]!=combo[b][k])==1:
                ...
                CV([projP(cw[a],ox,oy),projP(cw[b],ox,oy)], w if near else 1.0, INK if near else FAINT)
def taper(a,b,ra,rb,ox,oy,w=1.8,col=INK):
    a=V(*a); b=V(*b); ax=b-a
...
def limb(a,knee,b,r,ox,oy,foot=False):
    rh,rk,ra=r
    fillhull(seg_pts(a,knee,rh,rk),ox,oy); fillhull(seg_pts(knee,b,rk*0.92,ra),ox,oy); fillhull(sphere_pts(knee,rk),ox,oy)
    if foot:
        bb=V(*b); fy=(GROUND-bb[1]); fc=bb+V(0,fy*0.5,0.42); fh=(ra*0.95,max(fy*0.5,0.05),0.62)
        fillhull(box_pts(fc,fh,EYE),ox,oy)
    taper(a,knee,rh,rk,ox,oy); CV(hull([projP(p,ox,oy) for p in sphere_pts(knee,rk)]),1.3,INK,closed=True)
    taper(knee,b,rk*0.92,ra,ox,oy); ringp(a,rh,V(*knee)-V(*a),ox,oy)
    if foot: boxsolid(fc,fh,EYE,ox,oy,1.4)
def armseg(sh,el,wr,r,ox,oy):
    rs,re_,rw=r
    fillhull(sphere_pts(sh,rs),ox,oy); fillhull(seg_pts(sh,el,rs*0.85,re_),ox,oy)
    fillhull(sphere_pts(el,re_),ox,oy); fillhull(seg_pts(el,wr,re_*0.9,rw),ox,oy)
    CV(hull([projP(p,ox,oy) for p in sphere_pts(sh,rs)]),1.3,INK,closed=True); taper(sh,el,rs*0.85,re_,ox,oy)
    CV(hull([projP(p,ox,oy) for p in sphere_pts(el,re_)]),1.2,INK,closed=True); taper(el,wr,re_*0.9,rw,ox,oy)
def catmull(P):
    P=[V(*p) for p in P]; out=[]
    for i in range(len(P)-1):
        p0=P[max(i-1,0)]; p1=P[i]; p2=P[i+1]; p3=P[min(i+2,len(P)-1)]
        for t in np.linspace(0,1,14):
            t2=t*t;t3=t2*t; out.append(0.5*((2*p1)+(-p0+p2)*t+(2*p0-5*p1+4*p2-p3)*t2+(-p0+3*p1-3*p2+p3)*t3))
    return out
...
def chain_pts(spec):
    ...
    occiput=hc+hrot@V(0,hr[1]*0.55,-hr[2]*0.78); C7=(shL+shR)/2+rrot@V(0,-0.04,-0.12)
    midT=rc+rrot@V(0,-rr[1]*0.15,-rr[2]*0.72); T12=rc+rrot@V(0,rr[1]*0.92,-rr[2]*0.42)
    sacrum=pc+prot@V(0,-ph[1]*0.9,-ph[2]*0.6); lumbar=(T12+sacrum)/2+V(0,0,0.14)
    notch=rc+rrot@V(0,-rr[1]*0.74,rr[2]*0.55)
    return dict(spine=[occiput,C7,midT,T12,lumbar,sacrum],shL=shL,shR=shR,notch=notch,occ=occiput,C7=C7,sac=sacrum)
...
def figure(ox,oy,spec):
    ...
    ch=chain_pts(spec)
    # BACK -> FRONT (occlusion makes the masses overlap into one solid)
    CV([projP(p,ox,oy) for p in catmull(ch['spine'])],2.5,CHAIN)          # spine (back; torso will cover it)
    far_arm=spec['arms'][0]; near_arm=spec['arms'][1]
    far_leg=spec['legs'][0]; near_leg=spec['legs'][1]
    armseg(*far_arm,ox,oy)                                                # far arm (behind torso)
    limb(far_leg[0],far_leg[1],far_leg[2],far_leg[3],ox,oy,foot=True)
    limb(near_leg[0],near_leg[1],near_leg[2],near_leg[3],ox,oy,foot=True)
    boxsolid(*spec['pelvis'],ox,oy)                                       # pelvis (over leg tops)
    ellipsoid(*spec['rib'],ox,oy)                                         # ribcage (over pelvis + far arm + spine)
    nk=spec['neck']; taper(nk[0],nk[1],nk[2],nk[2]*0.95,ox,oy)
    ellipsoid(*spec['head'],ox,oy)                                        # head (over neck/ribcage top)
    CV([projP(ch['shL'],ox,oy),projP(ch['shR'],ox,oy)],2.1,CHAIN)         # shoulder line (front of chest)
    CV([projP(ch['shL'],ox,oy),projP(ch['notch'],ox,oy)],1.6,CHAIN); CV([projP(ch['shR'],ox,oy),projP(ch['notch'],ox,oy)],1.6,CHAIN)
    armseg(*near_arm,ox,oy)                                               # near arm (over ribcage/torso)
    for nd in (ch['occ'],ch['C7'],ch['sac'],ch['notch']):
...
D.text((46*SS,40*SS),'FIVE PEOPLE  ·  OVERLAP',fill=INK,font=F_TTL)
D.text((46*SS,74*SS),caps('same masses — now they occupy the same space. connection became an organism; overlap makes it solid.'),fill=LABEL,font=F_CAP)
...
D.text((46*SS,996*SS),'ribcage over pelvis, arm over ribcage, the far arm lost behind the torso — the parts now share one space.',fill=LABEL,font=F_IT)
out=img.resize((W,H),Image.LANCZOS); os.makedirs(OUT,exist_ok=True)
out.save(f'{OUT}/five_people_overlap.png'); print('done',out.size)
